...
class MovieLensPipelineTests(unittest.TestCase):
    def setUp(self) -> None:
        self.bundle = MovieLensBundle(
            ...
            users=pd.DataFrame(
                [
                    {"user_id": 1, "gender": "F", "age": 25, "occupation": 1, "zip_code": "12345"},
                    {"user_id": 2, "gender": "M", "age": 35, "occupation": 2, "zip_code": "54321"},
                ]
            ),
            movies=pd.DataFrame(
                [
                    {"movie_id": 10, "title": "Movie A (1999)", "genres": "Action|Comedy"},
    ...
    def test_profile_raw_data_identifica_pares_duplicados_e_referencias_invalidas(self) -> None:
    ...
    def test_clean_movielens_remove_interacao_invalida_e_mantem_rating_mais_recente(self) -> None:
        ...
        user_one_movie_ten = clean_bundle.ratings[
            (clean_bundle.ratings["user_id"] == 1) & (clean_bundle.ratings["movie_id"] == 10)
        ]
    ...
    def test_temporal_leave_last_k_out_separa_sem_vazamento_temporal(self) -> None:
        ...
        for user_id in feature_tables["interactions"]["user_id"].unique():
            user_train = splits["train"].loc[splits["train"]["user_id"] == user_id, "timestamp"]
            user_validation = splits["validation"].loc[
                splits["validation"]["user_id"] == user_id, "timestamp"
            ]
            user_test = splits["test"].loc[splits["test"]["user_id"] == user_id, "timestamp"]
